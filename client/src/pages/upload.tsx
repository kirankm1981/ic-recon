...
export default function UploadPage() {
  ...
  const [step, setStep] = useState<"select" | "mapping" | "done">("select");
  ...
  const uploadMutation = useMutation({
    mutationFn: async ({ file, mapping }: { file: File; mapping: Partial<ColumnMapping> }) => {
      const formData = new FormData();
      formData.append("file", file);
  ...
  const handleUpload = () => {
    ...
    uploadMutation.mutate({ file: selectedFile, mapping: columnMapping });
  };
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Import intercompany transaction files in CSV format
          </p>
        </div>
      ...
      {step === "mapping" && selectedFile && (
        <Card>
          ...
          <CardContent className="space-y-6">
            ...
            {sampleRows.length > 0 && (
              <div>
                <h3 className="text-sm font-medium mb-2">CSV Preview (first {sampleRows.length} rows)</h3>
