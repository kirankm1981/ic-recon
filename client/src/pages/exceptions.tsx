...
function formatDate(dateStr: string | null): string {
  ...
  if (!isNaN(num) && num > 40000 && num < 60000) {
    ...
    return epoch.toLocaleDateString("en-IN", { day: "2-digit", month: "short", year: "numeric" });
  }
  return dateStr;
}
...
export default function Exceptions() {
  ...
  const { data: transactions, isLoading } = useQuery<Transaction[]>({
    queryKey: ["/api/transactions", "reconStatus=unmatched"],
    queryFn: async () => {
      const res = await fetch("/api/transactions?reconStatus=unmatched");
                ...
                <table className="w-full text-sm" data-testid="table-exceptions">
                  <thead className="sticky top-0 bg-card z-10">
                    <tr className="border-b">
                      ...
                      <th className="text-right py-2.5 px-3 text-xs font-medium text-muted-foreground uppercase">Debit</th>
                      <th className="text-right py-2.5 px-3 text-xs font-medium text-muted-foreground uppercase">Credit</th>
                  ...
                  <tbody>
                    {filtered.map((txn) => (
                      ...
                      >
                        ...
                        <td className="py-2.5 px-3 text-right text-xs font-mono">{formatAmount(txn.debit)}</td>
                        <td className="py-2.5 px-3 text-right text-xs font-mono">{formatAmount(txn.credit)}</td>
